...
# '''old: was used to convert dict found online to reverse ris_labels dict required'''
...
def csv_to_ris(input, output, ris_labels):
    ...
    * create new file in ris format on a per row basis
    ...
    include_columns = ['type_of_reference', 'title', 'authors', 'secondary_title', 'date', 'year', 'doi', 'volume', 'number', 'issn', 'short_title', 'url', 'access_date', 'keywords', 'abstract', 'start_page', 'end_page', 'accession_number', 'asreview_ranking']  #asreview_ranking is included under ris label N1 - 'notes'
    # exclude_columns = ['record_id', 'database_provider', 'alternate_title1','language','secondary_authors', 'note', 'publisher']  # for reference
...
# Import ris labels dict from json
